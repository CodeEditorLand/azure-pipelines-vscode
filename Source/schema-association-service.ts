...
async function autoDetectSchema(
	...
	if (remoteUrl !== undefined && isAzureReposUrl(remoteUrl)) {
		...
		if (azurePipelinesDetails?.[workspaceFolder.name] !== undefined) {
			...
			const organizations = (
				await Promise.all(
					azureDevOpsSessions.map(async (session) => {
...
export async function getAzureDevOpsSessions(
	context: vscode.ExtensionContext,
	options?: vscode.AuthenticationGetSessionOptions,
): Promise<vscode.AuthenticationSession[] | undefined> {
	// First, request an ARM token.
	const managementSession = await vscode.authentication.getSession(
		"microsoft",
		AZURE_MANAGEMENT_SCOPES,
		options,
	);
	...
	while (nextLink !== undefined) {
		...
		for (const tenant of data.value) {
			const session = await vscode.authentication.getSession(
				"microsoft",
				[...AZURE_DEVOPS_SCOPES, `VSCODE_TENANT:${tenant.tenantId}`],
				{ silent: true },
			);
	...
	if (
		managementSession.account.id.includes("live.com") ||
		...
		const msaSession = await vscode.authentication.getSession(
			"microsoft",
			AZURE_DEVOPS_SCOPES,
			{ silent: true },
		);
