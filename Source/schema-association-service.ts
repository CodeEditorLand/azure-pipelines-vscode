...
async function autoDetectSchema(
	...
	if (!(await azureAccountApi.waitForSubscriptions())) {
		...
		try {
			...
			logger.log(
				`Error ${error} while trying to delete 1ESPTSchema folder. Either the folder does not exist or there is an actual error.`,
				"SchemaDetection"
			);
		}
	...
	try {
		...
		logger.log(
			`${workspaceFolder.name} has no remote URLs: ${error}`,
			"SchemaDetection"
		);
	}
	...
	if (
		remoteUrl !== undefined &&
		AzureDevOpsHelper.isAzureReposUrl(remoteUrl)
	) {
		logger.log(
...
export namespace SchemaAssociationNotification {
	export const type =
		new languageclient.NotificationType<ISchemaAssociations>(
