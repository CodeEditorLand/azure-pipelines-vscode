...
import {
	...
	isAzureReposUrl,
} from "./configure/helper/devOps/azureDevOpsHelper";
import type { QuickPickItemWithData } from "./configure/model/models";
import {
...
import type { AzureSession } from "./typings/azure-account.api";
...
export async function locateSchemaFile(
	...
	if (workspaceFolder !== undefined) {
		try {
			...
			logger.log(
				`Error auto-detecting schema for workspace folder ${
	...
	logger.log(
		`Using hardcoded schema for workspace folder ${
			workspaceFolder?.name ?? "ANONYMOUS_WORKSPACE"
		}: ${schemaUri.path}`,
		"SchemaDetection",
	);
...
async function autoDetectSchema(
	...
	if (!(await azureAccountApi.waitForSubscriptions())) {
		...
		try {
			...
			logger.log(
				`Error ${String(
					error,
				)} while trying to delete 1ESPTSchema folder. Either the folder does not exist or there is an actual error.`,
				"SchemaDetection",
			);
		}
	...
	if (remoteUrl !== undefined && isAzureReposUrl(remoteUrl)) {
		...
		if (azurePipelinesDetails?.[workspaceFolder.name] !== undefined) {
			...
			session = azureAccountApi.sessions.find(
				(session) => session.tenantId === details.tenant,
			);
			...
			logger.log(
				`Prompting for organization for ${workspaceFolder.name}`,
				"SchemaDetection",
			);
			...
			void vscode.window
				...
				.then(async (action) => {
					if (action === Messages.selectOrganizationLabel) {
						...
						const selectedOrganizationAndSession =
							await showQuickPick(
								"organization",
	...
	if (session === undefined) {
		logger.log(
			`No organization found for ${workspaceFolder.name}`,
			"SchemaDetection",
		);
	...
	logger.log(
		`Retrieving schema for ${workspaceFolder.name}`,
		"SchemaDetection",
	);
	...
	if (repoId1espt.length > 0) {
		...
		// If 1ESPT schema is not enabled, show a pop-up option to enable it for enhanced intellisense
			...
			context.globalState.get("doNotAskAgain1ESPTSchema") == undefined ||
	...
	if (seenOrganizations.has(organizationName)) {
		logger.log(
			`Returning cached schema for ${workspaceFolder.name}`,
			"SchemaDetection",
		);
		return schemaUri;
	}
