...
import {
	getRepositoryDetailsFromRemoteUrl,
	isAzureReposUrl,
} from "./helpers/azureDevOpsHelper";
import { showQuickPick } from "./helpers/controlProvider";
import * as logger from "./logger";
import * as Messages from "./messages";
import {
	delete1ESPTSchemaFileIfPresent,
	get1ESPTRepoIdIfAvailable,
	get1ESPTSchemaUri,
	getCached1ESPTSchema,
} from "./schema-association-service-1espt";
...
export async function locateSchemaFile(
	...
	let alternateSchema = vscode.workspace
		.getConfiguration("azure-pipelines")
		.get<string>("customSchemaFile", "");
	if (alternateSchema.trim().length === 0) {
		alternateSchema = path.join(
	...
	if (
		alternateSchema.toLowerCase().startsWith("http://") ||
		alternateSchema.toLowerCase().startsWith("https://")
	) {
		...
		schemaUri = vscode.Uri.file(
			path.join(workspaceFolder.uri.fsPath, alternateSchema),
		);
	} else {
		schemaUri = vscode.Uri.file(
			path.join(context.extensionPath, "service-schema.json"),
		);
	}
...
export function getSchemaAssociation(
...
async function autoDetectSchema(
	...
	try {
		...
		logger.log(
			`${workspaceFolder.name} has no remote URLs: ${String(error)}`,
			"SchemaDetection",
		);
	}
	...
	if (remoteUrl !== undefined && isAzureReposUrl(remoteUrl)) {
		logger.log(
			`${workspaceFolder.name} is an Azure repo`,
			"SchemaDetection",
		);
		...
		logger.log(
			`Using organization "${organizationName}"`,
			"SchemaDetection",
		);
	} else {
		logger.log(
			`${workspaceFolder.name} has no remote URL or is not an Azure repo`,
			"SchemaDetection",
		);
		...
		if (azurePipelinesDetails?.[workspaceFolder.name] !== undefined) {
			...
			organizationName =
				azurePipelinesDetails[workspaceFolder.name].organization;
			...
			const doNotAskAgainSelectOrg = context.globalState.get<boolean>(
				DO_NOT_ASK_SELECT_ORG_KEY,
			);
			if (doNotAskAgainSelectOrg) {
				logger.log(
					`Not prompting for organization - do not ask again was set`,
					"SchemaDetection",
				);
				return undefined;
			}
			...
			logger.log(
				`Retrieving organizations for ${workspaceFolder.name}`,
				"SchemaDetection",
			);

			const organizationsClient = new OrganizationsClient(
				azureDevOpsSession.accessToken,
			);
			const organizations = (
			...
			logger.log(
				`${organizations.length} organizations found - prompting for ${workspaceFolder.name}`,
				"SchemaDetection",
			);
			...
			void vscode.window
				.showInformationMessage(
					format(
						Messages.selectOrganizationForEnhancedIntelliSense,
				...
				.then(async (action) => {
					if (action === Messages.selectOrganizationLabel) {
						...
						quickPickItems.push({
							label: Messages.signInWithADifferentAccountLabel,
						});
						quickPickItems.push({
							label: Messages.changeTenantLabel,
						});
						...
						const selectedOrganization = await showQuickPick(
							...
							{
								placeHolder: format(
									Messages.selectOrganizationPlaceholder,
									workspaceFolder.name,
								),
							},
						);
						...
						if (selectedOrganization === undefined) {
							logger.log(
								`No organization picked for ${workspaceFolder.name}`,
								"SchemaDetection",
							);
							return;
						}
						...
						if (
							...
							logger.log(
								"Opening tenant settings",
								"SchemaDetection",
							);
							await vscode.env.openExternal(
								vscode.Uri.parse(
									`${vscode.env.uriScheme}://settings/${context.extension.id.split(".")[1]}.tenant`,
								),
							);
							return undefined;
						}
						...
						logger.log(
							`Associating organization "${organizationName}" with ${workspaceFolder.name}`,
							"SchemaDetection",
						);
	...
	if (
		!organizations
			.map(({ accountName }) => accountName)
			.includes(organizationName)
	) {
		logger.log(
			`Account does not have access to ${organizationName}`,
			"SchemaDetection",
		);
	...
	logger.log(
		`Retrieving ${organizationName} schema for ${workspaceFolder.name}`,
		"SchemaDetection",
	);
	...
	const azureDevOpsClient = new azdev.WebApi(
	...
	if (repoId1espt === undefined) {
		repoId1espt = await get1ESPTRepoIdIfAvailable(
			azureDevOpsClient,
			organizationName,
		);
	}
	...
	if (repoId1espt.length > 0) {
		...
		if (
			vscode.workspace
				.getConfiguration("azure-pipelines", workspaceFolder)
				.get<boolean>("1ESPipelineTemplatesSchemaFile", false)
		) {
			const cachedSchemaUri1ESPT = await getCached1ESPTSchema(
				context,
				organizationName,
				azureDevOpsSession,
				lastUpdated1ESPTSchema,
			);
			if (cachedSchemaUri1ESPT) {
				...
				const schemaUri1ESPT = await get1ESPTSchemaUri(
					azureDevOpsClient,
					organizationName,
					azureDevOpsSession,
					context,
		...
		// If 1ESPT schema is not enabled, show a pop-up option to enable it for enhanced intellisense
			...
			if (
				context.globalState.get("doNotAskAgain1ESPTSchema") ==
					undefined ||
				!context.globalState.get("doNotAskAgain1ESPTSchema")
			) {
				const schema1esptPopupResponse =
					await vscode.window.showInformationMessage(
						Messages.userEligibleForEnahanced1ESPTIntellisense,
						Messages.enable1ESPTSchema,
						Messages.doNotAskAgain,
					);
				if (schema1esptPopupResponse === Messages.enable1ESPTSchema) {
					await vscode.workspace
						.getConfiguration("azure-pipelines")
						.update(
							"1ESPipelineTemplatesSchemaFile",
							true,
							vscode.ConfigurationTarget.Workspace,
						);
				} else if (
					schema1esptPopupResponse === Messages.doNotAskAgain
				) {
					await context.globalState.update(
						"doNotAskAgain1ESPTSchema",
						true,
					);
				}
			}
		...
		logger.log(
			`User is not eligible for enhanced 1ESPT Schema Intellisense.`,
			"SchemaDetection",
		);
	}
	...
	const schemaUri = Utils.joinPath(
		context.globalStorageUri,
		`${organizationName}-schema.json`,
	);
	if (seenOrganizations.has(organizationName)) {
		logger.log(
			`Returning cached ${organizationName} schema for ${workspaceFolder.name}`,
			"SchemaDetection",
		);
		return schemaUri;
	}
...
export async function getAzureDevOpsSession(
	context: vscode.ExtensionContext,
	options?: vscode.AuthenticationGetSessionOptions,
): Promise<vscode.AuthenticationSession | undefined> {
	const rawTenantId = vscode.workspace
		.getConfiguration("azure-pipelines")
		.get<string>("tenant", "");
	const tenantId = rawTenantId.trim().length > 0 ? rawTenantId : undefined;
	logger.log(
		`Getting session for tenant ${tenantId ?? "default"}`,
		"SchemaDetection",
	);
	const azureDevOpsSession = await vscode.authentication.getSession(
		"microsoft",
		tenantId
			? [...AZURE_DEVOPS_SCOPES, `VSCODE_TENANT:${tenantId}`]
			: AZURE_DEVOPS_SCOPES,
		options,
	);
	if (azureDevOpsSession === undefined) {
		const doNotAskAgainSignIn = context.globalState.get<boolean>(
			DO_NOT_ASK_SIGN_IN_KEY,
		);
		if (doNotAskAgainSignIn) {
			logger.log(
				`Not prompting for login - do not ask again was set`,
				"SchemaDetection",
			);
			return undefined;
		}
		...
		try {
			...
			logger.log(
				`Error ${String(error)} while trying to delete 1ESPTSchema folder. Either the folder does not exist or there is an actual error.`,
				"SchemaDetection",
			);
		}
	...
	logger.log(
		`Retrieved session: ${azureDevOpsSession.account.id}`,
		"SchemaDetection",
	);
...
export const SchemaAssociationNotification = {
	type: new languageclient.NotificationType<ISchemaAssociations>(
