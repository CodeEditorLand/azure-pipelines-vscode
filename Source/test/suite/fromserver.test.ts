...
suite("Validation Tests From Server", async function () {
	...
	test("Validate all files from server", async function () {
		const validFiles: vscode.Uri[] =
			await vscode.workspace.findFiles("**/extracted/*.yml");
...
async function testFileIsValid(file: vscode.Uri) {
	// Arrange and Act
	const emptyDocument: vscode.TextDocument =
	...
	const diagnostics: vscode.Diagnostic[] =
		vscode.languages.getDiagnostics(file);
	...
	assert.strictEqual(
		diagnostics.length,
		0,
		"File: " + file.path + " Error: " + JSON.stringify(diagnostics)
	);
}
