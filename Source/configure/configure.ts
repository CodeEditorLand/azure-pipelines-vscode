...
class PipelineConfigurer {
	...
	private async createPreRequisites(): Promise<void> {
		if (this.inputs.isNewOrganization) {
			this.inputs.project = {
				id: "",
				name: AzureDevOpsHelper.generateDevOpsProjectName(
					this.inputs.sourceRepository.repositoryName
				),
			};
			await vscode.window.withProgress(
				{
				...
				async () => {
					try {
						...
						await operationsClient.waitForOperationSuccess(
							operation.id
						);
						this.inputs.project = await coreApi.getProject(
		...
		if (
			this.inputs.pipelineParameters.pipelineTemplate.targetType !=
	...
	private async getGitDetailsFromRepository(): Promise<void> {
		...
		this.inputs.sourceRepository = await this.getGitRepositoryParameters(
			name,
			remote
		);
	...
	private async getGitHubPatToken(): Promise<string> {
		return await telemetryHelper.executeFunctionWithTimeTelemetry(
			async () => {
				return await showInputBox(constants.GitHubPat, {
	...
	private async getAzureDevOpsDetails(): Promise<void> {
		try {
			this.organizationsClient = new OrganizationsClient(
				this.inputs.azureSession.credentials2
			);
			if (
				this.inputs.sourceRepository.repositoryProvider ===
				...
				let devOpsOrganizations =
					await this.organizationsClient.listOrganizations();
				...
				if (devOpsOrganizations && devOpsOrganizations.length > 0) {
					let selectedOrganization = await showQuickPick(
						...
						devOpsOrganizations.map((organization) => {
							return { label: organization.accountName };
						}),
					...
					let userName = this.inputs.azureSession.userId.substring(
						0,
						this.inputs.azureSession.userId.indexOf("@")
					);
					let organizationName =
						AzureDevOpsHelper.generateDevOpsOrganizationName(
							userName,
							this.inputs.sourceRepository.repositoryName
						);

					let validationErrorMessage =
						await this.organizationsClient.validateOrganizationName(
							organizationName
						);
						...
						this.inputs.organizationName = await showInputBox(
							constants.EnterOrganizationName,
							{
								...
								validateInput: (organizationName) =>
									this.organizationsClient.validateOrganizationName(
										organizationName
									),
							}
						);
	...
	private async getAzureResourceDetails(): Promise<void> {
		...
		const subscriptionList = azureAccountApi.filters.map(
			(subscriptionObject) => {
		...
		if (
			...
			this.inputs.targetResource.subscriptionId =
			...
			this.appServiceClient = new AppServiceClient(
				this.inputs.azureSession.credentials2,
				this.inputs.azureSession.tenantId,
				this.inputs.azureSession.environment.portalUrl,
				this.inputs.targetResource.subscriptionId
			);

			let resourceArray: Promise<
				Array<{
					label: string;
					data: ResourceManagementModels.GenericResource;
				}>
			> = null;
			...
			switch (
				...
				default:
					resourceArray = this.appServiceClient
						.GetAppServices(
							this.inputs.pipelineParameters.pipelineTemplate
								.targetKind
						)
					...
					selectAppText = this.getSelectAppText(
						this.inputs.pipelineParameters.pipelineTemplate
							.targetKind
					);
			...
			const selectedResource: QuickPickItemWithData<ResourceManagementModels.GenericResource> =
				await showQuickPick(
					selectAppText,
					resourceArray,
			...
			this.inputs.targetResource.subscriptionId =
				subscriptionList[0].data.subscription.subscriptionId;
			this.inputs.azureSession = await getSubscriptionSession(
				this.azureAccount,
				this.inputs.targetResource.subscriptionId
			);
		...
		try {
			...
			let buildDefinitionUrl =
				AzureDevOpsHelper.getOldFormatBuildDefinitionUrl(
					this.inputs.organizationName,
					this.inputs.project.id,
					queuedPipeline.definition.id
				);
			let buildUrl = AzureDevOpsHelper.getOldFormatBuildUrl(
				this.inputs.organizationName,
				this.inputs.project.id,
				queuedPipeline.id
			);
			...
			let metadata = await this.appServiceClient.getAppServiceMetadata(
				this.inputs.targetResource.resource.id
			);
	...
	private async createGithubServiceConnection(): Promise<void> {
		if (!this.serviceConnectionHelper) {
			this.serviceConnectionHelper = new ServiceConnectionHelper(
				this.inputs.organizationName,
				this.inputs.project.name,
		...
		await vscode.window.withProgress(
			...
			async () => {
				try {
					...
					this.inputs.sourceRepository.serviceConnectionId =
						await this.serviceConnectionHelper.createGitHubServiceConnection(
	...
	private async createAzureRMServiceConnection(): Promise<void> {
		...
		this.inputs.targetResource.serviceConnectionId =
			await vscode.window.withProgress(
				{
					location: vscode.ProgressLocation.Notification,
					title: utils.format(
				...
				async () => {
					try {
						...
						return await this.serviceConnectionHelper.createAzureServiceConnection(
							serviceConnectionName,
							this.inputs.azureSession.tenantId,
							this.inputs.targetResource.subscriptionId,
							scope,
	...
	private async checkInPipelineFileToRepository(): Promise<void> {
		try {
			this.inputs.pipelineParameters.pipelineFileName =
				await getAvailableFileName(
					"azure-pipelines.yml",
					this.workspaceUri
				);
			const fileUri = Utils.joinPath(
				this.workspaceUri,
				this.inputs.pipelineParameters.pipelineFileName
			);
				...
				const commitOrDiscard =
					await vscode.window.showInformationMessage(
						utils.format(
							...
							this.inputs.sourceRepository.remoteName
						),
						Messages.commitAndPush,
					...
					await vscode.window.withProgress(
						...
						async (progress) => {
							try {
								...
								this.inputs.sourceRepository.commitId =
									this.repo.state.HEAD.commit;
							} catch (error) {
								telemetryHelper.logError(
									Layer,
	...
	private async createAndRunPipeline(): Promise<Build> {
		return await vscode.window.withProgress(
			...
			async () => {
				try {
					const taskAgentApi =
						await this.azureDevOpsClient.getTaskAgentApi();
					...
					const pipelineName = `${
						this.inputs.targetResource.resource
							? this.inputs.targetResource.resource.name
							: this.inputs.pipelineParameters.pipelineTemplate
									.label
	...
	private async getAzureDevOpsClient(): Promise<azdev.WebApi> {
		...
		this.azureDevOpsClient = new azdev.WebApi(
			`https://dev.azure.com/${this.inputs.organizationName}`,
			authHandler
		);
		return this.azureDevOpsClient;
	}
