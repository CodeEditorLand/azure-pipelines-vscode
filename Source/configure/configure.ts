...
import type { AzureSiteDetails } from "./model/models";
...
class PipelineConfigurer {
	...
	public async configure(): Promise<void> {
		...
		if (template.target.type !== TargetResourceType.None) {
			azureSiteDetails = await this.getAzureResourceDetails(
				adoDetails.session,
		...
		const commit = await this.checkInPipelineFileToRepository(
			pipelineFileName,
			repoDetails,
		);
		if (commit === undefined) {
			...
			.then((action) => {
				if (action === Messages.browsePipeline) {
					...
					void vscode.env.openExternal(
						URI.parse(queuedPipeline._links.web.href),
					);
				}
			});
	}

	private async getGitDetailsFromRepository(): Promise<
		GitRepositoryDetails | undefined
	> {
		...
		let remoteUrl = this.repo.state.remotes.find(
			(remoteObj) => remoteObj.name === remote,
		)?.fetchUrl;
		if (remoteUrl !== undefined) {
			if (AzureDevOpsHelper.isAzureReposUrl(remoteUrl)) {
				remoteUrl = AzureDevOpsHelper.getFormattedRemoteUrl(remoteUrl);
				const { organizationName, projectName, repositoryName } =
				...
				const { ownerName, repositoryName } =
					GitHubHelper.getRepositoryDetailsFromRemoteUrl(remoteUrl);
				repoDetails = {
				...
				void vscode.window.showWarningMessage(
					Messages.cannotIdentifyRepositoryDetails,
				);
				return undefined;
			}
		} else {
			void vscode.window.showWarningMessage(
				Messages.remoteRepositoryNotConfigured,
			);
			return undefined;
		}
		...
		telemetryHelper.setTelemetry(
			...
			repoDetails.repositoryProvider,
		);
	...
	private async getSelectedPipeline(): Promise<PipelineTemplate | undefined> {
		const appropriateTemplates: PipelineTemplate[] =
		...
		const template = await showQuickPick(
			...
			appropriateTemplates.map((template) => {
				return { label: template.label, data: template };
			}),
	...
	): Promise<AzureDevOpsDetails | undefined> {
		if (repoDetails.repositoryProvider === RepositoryProvider.AzureRepos) {
			for (const session of this.azureAccount.filters.map(
				({ session }) => session,
			)) {
				...
				const organizations =
					await organizationsClient.listOrganizations();
				if (
					organizations.find(
						(org) =>
							org.accountName.toLowerCase() ===
							repoDetails.organizationName.toLowerCase(),
					)
				) {
					const adoClient = await this.getAzureDevOpsClient(
						repoDetails.organizationName,
						session,
					);
			...
			void vscode.window.showWarningMessage(
				"You are not signed in to the Azure DevOps organization that contains this repository.",
			);
			return undefined;
		} else {
			...
			const getOrganizationsAndSessions = async (): Promise<
				QuickPickItemWithData<AzureSession | undefined>[]
			> => {
				return [
					...(
						await Promise.all(
							this.azureAccount.filters.map(
								async ({ session }) => {
			...
			const result = await showQuickPick(
				...
				{
					placeHolder:
						"Select the Azure DevOps organization to create this pipeline in",
				},
			...
			if (session === undefined) {
				...
				await vscode.env.openExternal(
					vscode.Uri.parse("https://dev.azure.com/"),
				);
				return undefined;
			}
			...
			> => {
				...
				return [
					...projects.filter(isValidProject).map((project) => {
						return { label: project.name, data: project };
					}),
					{
	...
	private async getAzureResourceDetails(
		...
		kind: WebAppKind,
	): Promise<AzureSiteDetails | undefined> {
		...
		const subscriptionList = this.azureAccount.filters
			...
			.map((subscriptionObject) => {
				return {
					label:
						subscriptionObject.subscription.displayName ??
						"Unknown subscription",
					data: subscriptionObject,
					description:
						subscriptionObject.subscription.subscriptionId ??
						undefined,
				};
			});
		...
		if (subscriptionId === undefined) {
			void vscode.window.showErrorMessage(
				"Unable to get ID for subscription, please file a bug at https://github.com/microsoft/azure-pipelines-vscode/issues/new",
			);
			return undefined;
		}
		...
		const appType = kind.includes("functionapp")
			? "Function App"
			: "Web App";
		...
		if (site === undefined) {
			...
			const packageId = kind.includes("functionapp")
				? "Microsoft.FunctionApp"
				: "Microsoft.WebSite";
			await vscode.env.openExternal(
				vscode.Uri.parse(
					`https://portal.azure.com/#create/${packageId}`,
				),
			);
			return undefined;
		}
	...
	private async createGitHubServiceConnection(
		...
		return vscode.window.withProgress(
			...
			async () => {
				...
				try {
					return serviceConnectionHelper.createGitHubServiceConnection(
	...
	private async createPipelineFile(
		...
		try {
			const pipelineFileName = await getAvailableFileName(
				"azure-pipelines.yml",
				this.workspaceUri,
			);
				...
				return vscode.window.withProgress(
					...
					async () => {
						try {
							...
							if (commit === undefined) {
								void vscode.window.showErrorMessage(
									"Unable to get commit after pushing pipeline, please file a bug at https://github.com/microsoft/azure-pipelines-vscode/issues/new",
								);
								return undefined;
							}
	...
	private async createAndRunPipeline(
		...
		return vscode.window.withProgress(
			...
			async () => {
				try {
					const taskAgentApi =
						await adoDetails.adoClient.getTaskAgentApi();
					...
					const pipelineName = `${
						azureSiteDetails?.site.name ?? template.label
					}-${this.uniqueResourceNameSuffix}`;
					const definitionPayload =
						AzureDevOpsHelper.getBuildDefinitionPayload(
							pipelineName,
							queues[0],
							repoDetails,
							adoDetails,
					...
					const definition = await buildApi.createDefinition(
						definitionPayload,
						adoDetails.project.name,
					);
		...
		await vscode.window.withProgress(
			...
			async () => {
				try {
					...
					const buildDefinitionUrl =
						AzureDevOpsHelper.getOldFormatBuildDefinitionUrl(
							adoDetails.organizationName,
					...
					const locationsApi =
						await adoDetails.adoClient.getLocationsApi();
					const { instanceId } =
						await locationsApi.getConnectionData();
					if (instanceId === undefined) {
						void vscode.window.showErrorMessage(
							"Unable to determine the organization ID, please file a bug at https://github.com/microsoft/azure-pipelines-vscode/issues/new",
						);
						return;
					}
					...
					const metadata =
						await azureSiteDetails.appServiceClient.getAppServiceMetadata(
							azureSiteDetails.site,
						);
					metadata.properties = {
						...metadata.properties,
						...
						VSTSRM_BuildDefinitionId:
							queuedPipeline.definition.id.toString(),
						VSTSRM_BuildDefinitionWebAccessUrl: buildDefinitionUrl,
						VSTSRM_ConfiguredCDEndPoint: "",
						VSTSRM_ReleaseDefinitionId: "",
					};

					await azureSiteDetails.appServiceClient.updateAppServiceMetadata(
						azureSiteDetails.site,
						metadata,
					);
	...
	private async getAzureDevOpsClient(
		organization: string,
		session: AzureSession,
	): Promise<WebApi> {
		...
		this.azureDevOpsClient = new WebApi(
...
function isValidProject(project: TeamProject): project is ValidatedProject {
	if (project.name === undefined || project.id === undefined) {
		void vscode.window.showErrorMessage(
			"Unable to get name or ID for project, please file a bug at https://github.com/microsoft/azure-pipelines-vscode/issues/new",
		);
		return false;
	}
...
function isValidSite(
	resource: WebSiteManagementModels.Site,
): resource is ValidatedSite {
	if (resource.name === undefined || resource.id === undefined) {
		void vscode.window.showErrorMessage(
			"Unable to get name or ID for resource, please file a bug at https://github.com/microsoft/azure-pipelines-vscode/issues/new",
		);
		return false;
	}
...
function isValidBuild(build: Build): build is ValidatedBuild {
	if (
		build.definition === undefined ||
		build.definition.id === undefined ||
		build.id === undefined
	) {
		void vscode.window.showErrorMessage(
			"Unable to get definition or ID for build, please file a bug at https://github.com/microsoft/azure-pipelines-vscode/issues/new",
		);
		return false;
	}
