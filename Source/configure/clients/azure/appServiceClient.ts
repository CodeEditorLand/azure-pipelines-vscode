import {
	...
	type WebSiteManagementModels,
} from "@azure/arm-appservice";
import type { TokenCredentialsBase } from "@azure/ms-rest-nodeauth";
import { v4 as uuid } from "uuid";

import type { ValidatedSite, WebAppKind } from "../../model/models";
...
export class AppServiceClient {
	...
	public async getAppServices(
		filterForResourceKind: WebAppKind,
	): Promise<WebSiteManagementModels.Site[]> {
	...
	public async getAppServiceConfig(
		site: ValidatedSite,
	): Promise<WebSiteManagementModels.SiteConfigResource> {
	...
	public async getAppServiceMetadata(
		site: ValidatedSite,
	): Promise<WebSiteManagementModels.StringDictionary> {
	...
	public async updateAppServiceMetadata(
		site: ValidatedSite,
		metadata: WebSiteManagementModels.StringDictionary,
	): Promise<WebSiteManagementModels.StringDictionary> {
	...
	): Promise<WebSiteManagementModels.Deployment> {
		// create deployment object
		const deploymentId = uuid();
		const deployment = this.createDeploymentObject(
			deploymentId,
			buildDefinitionUrl,
	...
	private createDeploymentObject(
		...
		releaseDefinitionUrl: string,
		triggeredBuildUrl: string,
	): WebSiteManagementModels.Deployment {
		const message: DeploymentMessage = {
			type: "CDDeploymentConfiguration",
			message:
				"Successfully set up continuous delivery from VS Code and triggered deployment to Azure Web App.",
