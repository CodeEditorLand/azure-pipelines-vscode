...
import {
	OperationReference,
	OperationStatus,
} from "azure-devops-node-api/interfaces/common/OperationsInterfaces";
...
export class OperationsClient {
	...
	public async waitForOperationSuccess(operationId: string): Promise<void> {
		...
		for (let attempt = 0; attempt < 30; attempt++) {
			const response = await this.connection.rest.get<OperationReference>(
				url,
				{
			...
			if (operation.status === OperationStatus.Failed) {
				...
				throw new Error(
					util.format(
						Messages.failedToCreateAzureDevOpsProject,
						(operation as any).detailedMessage
					)
				);
			}
