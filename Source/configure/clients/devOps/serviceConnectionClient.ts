...
import type { AadApplication } from "../../model/models";
...
export class ServiceConnectionClient {
	...
	public async createGitHubServiceConnection(
		...
		const response = await this.connection.rest.create<ServiceConnection>(
			url,
			{
		...
		if (response.result) {
			...
			throw new Error(
				`Failed to create GitHub service connection: ${response.statusCode}`,
			);
		}
	}
	...
	public async createAzureServiceConnection(
		...
		if (response.result) {
			...
			throw new Error(
				`Failed to create Azure service connection: ${response.statusCode}`,
			);
		}
	}

	public async getEndpointStatus(
		endpointId: string,
	): Promise<ServiceConnection> {
		const url = `${this.connection.serverUrl}/${this.project}/_apis/serviceendpoint/endpoints/${endpointId}`;

		const response = await this.connection.rest.get<ServiceConnection>(
			url,
			{
				acceptHeader:
					"application/json;api-version=5.1-preview.2;excludeUrls=true",
		...
		if (response.result) {
			...
			throw new Error(
				`Failed to get service connection status: ${response.statusCode}`,
			);
		}
	}
	...
	public async authorizeEndpointForAllPipelines(
		...
		const response = await this.connection.rest.update<ServiceConnection>(
			url,
			{
				allPipelines: {
					authorized: true,
		...
		if (response.result) {
			...
			throw new Error(
				`Failed to authorize service connection: ${response.statusCode}`,
			);
		}
	}
}
