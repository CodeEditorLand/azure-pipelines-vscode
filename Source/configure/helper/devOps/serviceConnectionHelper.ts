...
export class ServiceConnectionHelper {
	...
	public async createGitHubServiceConnection(
		...
		const authorizationResponse =
			await this.serviceConnectionClient.authorizeEndpointForAllPipelines(
				endpointId,
			);
	...
	public async createAzureServiceConnection(
		...
		const connection =
			await this.serviceConnectionClient.createAzureServiceConnection(
				name,
				tenantId,
				subscriptionId,
				scope,
	...
	private async waitForEndpointToBeReady(endpointId: string): Promise<void> {
		for (let attempt = 0; attempt < 30; attempt++) {
			...
			if (operationStatus.state.toLowerCase() === "failed") {
				throw Error(
					util.format(
						Messages.unableToCreateServiceConnection,
						connection.type,
