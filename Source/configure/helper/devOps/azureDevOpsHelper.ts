...
import type { TaskAgentQueue } from "azure-devops-node-api/interfaces/TaskAgentInterfaces";
...
export function isAzureReposUrl(remoteUrl: string): boolean {
	return (
		remoteUrl.includes(AzureReposUrl) ||
...
export function getRepositoryDetailsFromRemoteUrl(remoteUrl: string): {
	...
	if (remoteUrl.includes(AzureReposUrl)) {
		const part = remoteUrl.substring(
			remoteUrl.indexOf(AzureReposUrl) + AzureReposUrl.length,
		);
		...
		const part = remoteUrl.substring(
			remoteUrl.indexOf(VSOUrl) + VSOUrl.length,
		);
		const organizationName = remoteUrl.substring(
		...
		const urlFormat = remoteUrl.includes(SSHAzureReposUrl)
			? SSHAzureReposUrl
			: SSHVsoReposUrl;
		const part = remoteUrl.substring(
...
export function getBuildDefinitionPayload(
	...
	return {
		...
		repository: {
			id:
				repoDetails.repositoryProvider === RepositoryProvider.Github
...
export function getOldFormatBuildDefinitionUrl(
	accountName: string,
	projectName: string,
...
export function getOldFormatBuildUrl(
	accountName: string,
	projectName: string,
	buildId: number,
) {
