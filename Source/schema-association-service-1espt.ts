...
export async function get1ESPTSchemaUri(
	...
	try {
		...
		logger.log(
			`Error : ${error} while fetching 1ESPT schema for org: ${organizationName}  : `,
			"SchemaDetection"
		);
	}
	return undefined;
...
export async function getCached1ESPTSchema(
	...
	seen1ESPTOrganizations: Set<string>
): Promise<URI> {
	if (seen1ESPTOrganizations.has(organizationName)) {
		...
		try {
			if (session.userId.endsWith("@microsoft.com")) {
				if (
					new Date().getTime() -
						lastUpdated1ESPTSchema.get(organizationName).getTime() <
					milliseconds24hours
				) {
			...
			logger.log(
				`Error : ${error} while fetching cached 1ESPT schema for org: ${organizationName}. It's possible that the schema does not exist.`,
				"SchemaDetection"
			);
		}
	}
...
export async function get1ESPTRepoIdIfAvailable(
	...
	try {
		...
		logger.log(
			`Error : ${error} while checking eligibility for enhanced Intellisense for 1ESPT schema for org: ${organizationName}.`,
			"SchemaDetection"
		);
		return "";
	}
...
export async function delete1ESPTSchemaFileIfPresent(
	...
	try {
		...
		logger.log(
			`Error: ${error} while deleting 1ESPT schema. It's possible that the schema file does not exist`,
			"SchemaDetection"
		);
	}
}
