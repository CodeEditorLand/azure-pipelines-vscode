...
export async function get1ESPTSchemaUri(
	...
	try {
		...
		logger.log(
			`Error: ${error instanceof Error ? error.message : String(error)} while fetching 1ESPT schema for org: ${organizationName}  : `,
			"SchemaDetection",
		);
	}
...
export async function getCached1ESPTSchema(
	...
	try {
		...
		logger.log(
			`Error: ${error instanceof Error ? error.message : String(error)} while fetching cached 1ESPT schema for org: ${organizationName}. It's possible that the schema does not exist.`,
			"SchemaDetection",
		);
	}
...
export async function get1ESPTRepoIdIfAvailable(
	...
	try {
		...
		logger.log(
			`Error: ${error instanceof Error ? error.message : String(error)} while checking eligibility for enhanced Intellisense for 1ESPT schema for org: ${organizationName}.`,
			"SchemaDetection",
		);
...
export async function delete1ESPTSchemaFileIfPresent(
	...
	try {
		...
		logger.log(
			`Error: ${error instanceof Error ? error.message : String(error)} while deleting 1ESPT schema. It's possible that the schema file does not exist`,
			"SchemaDetection",
		);
	}
}
