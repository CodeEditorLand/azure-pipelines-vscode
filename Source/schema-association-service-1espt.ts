...
export async function get1ESPTSchemaUri(
	azureDevOpsClient: azdev.WebApi,
	...
	try {
		if (session.userId.endsWith("@microsoft.com")) {
			...
			const schemaFile = await gitApi.getItem(
				repoId1espt,
				"schema/1espt-base-schema.json",
			...
			if (content === undefined) {
				logger.log(
					`File was retrieved without content for org: ${organizationName}`,
					"SchemaDetection",
				);
				return undefined;
			}
		...
		logger.log(
			`Error : ${String(error)} while fetching 1ESPT schema for org: ${organizationName}  : `,
			"SchemaDetection",
		);
	}
	return undefined;
 ...
 * @returns
 */
export async function getCached1ESPTSchema(
	context: vscode.ExtensionContext,
	organizationName: string,
	session: AzureSession,
	lastUpdated1ESPTSchema: Map<string, Date>,
): Promise<URI | undefined> {
	...
	const schemaUri1ESPT = Utils.joinPath(
		context.globalStorageUri,
		"1ESPTSchema",
		`${organizationName}-1espt-schema.json`,
	);
	...
	try {
		if (session.userId.endsWith("@microsoft.com")) {
			...
			// schema is older than 24 hours, fetch schema file again
				...
				logger.log(
					`Skipping cached 1ESPT schema for ${organizationName} as it is older than 24 hours`,
					`SchemaDetection`,
				);
			}
		} else {
			const signInAction = await vscode.window.showInformationMessage(
				Messages.notUsing1ESPTSchemaAsUserNotSignedInMessage,
		...
		logger.log(
			`Error : ${String(error)} while fetching cached 1ESPT schema for org: ${organizationName}. It's possible that the schema does not exist.`,
			"SchemaDetection",
		);
	}
 ...
 * @returns
 */
export async function get1ESPTRepoIdIfAvailable(
	azureDevOpsClient: azdev.WebApi,
	organizationName: string,
): Promise<string> {
	try {
		...
		if (repositories.length === 0) {
			logger.log(
				`1ESPipelineTemplates ADO project not found for org ${organizationName}`,
				`SchemaDetection`,
			);
		...
		const repository = repositories.find(
			(repo) => repo.name === "1ESPipelineTemplates",
		);
		if (repository?.id === undefined) {
			logger.log(
				`1ESPipelineTemplates repo not found for org ${organizationName}`,
				`SchemaDetection`,
			);
		...
		logger.log(
			`Error : ${String(error)} while checking eligibility for enhanced Intellisense for 1ESPT schema for org: ${organizationName}.`,
			"SchemaDetection",
		);
		return "";
	}
...
export async function delete1ESPTSchemaFileIfPresent(
	context: vscode.ExtensionContext,
) {
	try {
		...
		logger.log(
			`Error: ${String(error)} while deleting 1ESPT schema. It's possible that the schema file does not exist`,
			"SchemaDetection",
		);
	}
}
