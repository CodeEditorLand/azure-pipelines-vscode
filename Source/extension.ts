...
export async function activate(context: vscode.ExtensionContext) {
	const configurePipelineEnabled = vscode.workspace
		.getConfiguration(LANGUAGE_IDENTIFIER)
		.get<boolean>("configure", true);
	telemetryHelper.setTelemetry("isActivationEvent", "true");
	telemetryHelper.setTelemetry(
		"configurePipelineEnabled",
		`${configurePipelineEnabled}`,
	);
	await telemetryHelper.callWithTelemetryAndErrorHandling(
		...
		async () => {
			...
			if (configurePipelineEnabled) {
				const { activateConfigurePipeline } = await import(
					"./configure/activate"
				);
...
async function activateYmlContributor(context: vscode.ExtensionContext) {
	...
	vscode.languages.setLanguageConfiguration(LANGUAGE_IDENTIFIER, {
		wordPattern: /("(?:[^\\"]*(?:\\.)?)*"?)|[^\s{}[\],:]+/,
	});
	...
	context.subscriptions.push(
		azureAccountApi.onSessionsChanged(async () => {
			if (
				azureAccountApi.status === "LoggedIn" ||
				azureAccountApi.status === "LoggedOut"
			) {
				await loadSchema(context, client);
			}
		}),
	...
	context.subscriptions.push(
		onDidSelectOrganization(async (workspaceFolder) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"azure-pipelines.reset-state",
			async () => await resetDoNotAskState(context),
		),
	);
}
