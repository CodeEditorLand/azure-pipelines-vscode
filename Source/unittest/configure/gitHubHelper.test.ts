...
suite("GitHub Helper", () => {
	suite("isGitHubUrl", () => {
		...
		test("Returns false for non-GitHub HTTPS URLs", () => {
			assert.strictEqual(
				GitHubProvider.isGitHubUrl(
					"https://github.coms/microsoft/azure-pipelines-vscode"
				),
				false
			);
		});
		...
		test("Returns false for non-GitHub SSH URLs", () => {
			assert.strictEqual(
				GitHubProvider.isGitHubUrl(
					"sgit@github.com:microsoft/azure-pipelines-vscode.git"
				),
				false
			);
		});
	...
	suite("getFormattedRemoteUrl", () => {
		test("Returns HTTPS URLs as-is", () => {
			assert.strictEqual(
				GitHubProvider.getFormattedRemoteUrl(
					"https://github.com/microsoft/azure-pipelines-vscode.git"
				),
				"https://github.com/microsoft/azure-pipelines-vscode.git"
			);
