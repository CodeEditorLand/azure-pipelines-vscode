...
suite("Azure DevOps Helper", () => {
	suite("isAzureReposUrl", () => {
		test("Returns true for HTTPS ADO URLs", () => {
			assert.ok(
				AzureDevOpsHelper.isAzureReposUrl(
		...
		test("Returns true for SSH ADO URLs", () => {
			assert.ok(
				AzureDevOpsHelper.isAzureReposUrl(
					"git@ssh.dev.azure.com:v3/ms/example/repo"
				)
			);
		});
		...
		test("Returns true for legacy HTTPS VSTS URLs with DefaultCollection", () => {
			assert.ok(
				AzureDevOpsHelper.isAzureReposUrl(
					"https://ms.visualstudio.com/DefaultCollection/example/_git/repo"
				)
			);
		});
		...
		test("Returns true for legacy SSH VSTS URLs", () => {
			assert.ok(
				AzureDevOpsHelper.isAzureReposUrl(
					"ms@vs-ssh.visualstudio.com:v3/ms/example/repo"
				)
			);
		});
		...
		test("Returns false for non-ADO HTTPS URLs", () => {
			assert.strictEqual(
				AzureDevOpsHelper.isAzureReposUrl(
					"https://dev.azure.coms/ms/example/_git/repo"
				),
				false
			);
		});
		...
		test("Returns false for non-ADO SSH URLs", () => {
			assert.strictEqual(
				AzureDevOpsHelper.isAzureReposUrl(
					"git@dev.azure.com:v3/ms/example/repo"
				),
				false
			);
		});
		...
		test("Returns false for non-VSTS HTTPS URLs", () => {
			assert.strictEqual(
				AzureDevOpsHelper.isAzureReposUrl(
					"https://ms.visualstudio.coms/example/_git/repo"
				),
				false
			);
		});
		...
		test("Returns false for non-VSTS SSH URLs", () => {
			assert.strictEqual(
				AzureDevOpsHelper.isAzureReposUrl(
					"ms@ssh.visualstudio.com:v3/ms/example/repo"
				),
				false
			);
		});
	...
	suite("getFormattedRemoteUrl", () => {
		test("Returns HTTPS ADO URLs as-is", () => {
			assert.strictEqual(
				AzureDevOpsHelper.getFormattedRemoteUrl(
					"https://dev.azure.com/ms/example/_git/repo"
				),
				"https://dev.azure.com/ms/example/_git/repo"
			);
	...
	suite("generateDevOpsProjectName", () => {
		...
		test("Strips trailing periods from repository name", () => {
			assert.strictEqual(
				AzureDevOpsHelper.generateDevOpsProjectName(
					"owner/repos.itory..."
				),
		...
		test("Keeps final project name within 64 characters", () => {
			...
			const projectName =
				"AzurePipelines-0123456789012345678901234567890123456789012345678";
