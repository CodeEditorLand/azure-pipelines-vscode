...
class TelemetryHelper {
	...
	public logInfo(layer: string, tracePoint: string, info: string): void {
		TelemetryHelper.reporter.sendTelemetryEvent(tracePoint, {
			[TelemetryKeys.JourneyId]: this.journeyId,
			"command": this.command,
			"layer": layer,
			"info": info,
	...
	public async callWithTelemetryAndErrorHandling<T>(
		callback: () => Promise<T>
	): Promise<T | void> {
		try {
			return await this.executeFunctionWithTimeTelemetry(
				callback,
				"duration"
			);
		} catch (error) {
			...
			if (parsedError.isUserCancelledError) {
				...
				if (parsedError.message.includes("\n")) {
					vscode.window.showErrorMessage(
						"An error has occurred. Check the output window for more details."
					);
				} else {
			...
			if (this.properties.result === Result.Failed) {
				TelemetryHelper.reporter.sendTelemetryErrorEvent(
					...
					undefined,
					["error", "errorMesage", "stack"]
				);
			} else if (
				!(
					this.options.suppressIfSuccessful &&
					this.properties.result === Result.Succeeded
				)
			) {
				TelemetryHelper.reporter.sendTelemetryEvent(this.command, {
					...this.properties,
					[TelemetryKeys.JourneyId]: this.journeyId,
				});
			}
